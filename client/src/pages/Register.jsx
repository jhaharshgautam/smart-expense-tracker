...
export default function Register() {
  ...
  const handleSubmit = async e => {
    ...
    try { const res=await API.post('/auth/register',form); login(res.data.user,res.data.token); navigate('/dashboard'); }
  ...
  return (
    <div style={{fontFamily:"'DM Sans',sans-serif",background:'#080810',minHeight:'100vh',display:'flex',alignItems:'center',justifyContent:'center',position:'relative',overflow:'hidden',color:'#f0f0ff'}}>
      ...
      <div style={{position:'relative',zIndex:1,width:'100%',maxWidth:440,background:'linear-gradient(160deg,#151528,#0f0f1e)',border:'1px solid rgba(155,109,255,0.2)',borderRadius:28,padding:'48px 40px',boxShadow:'0 0 80px rgba(124,92,191,0.12)'}}>
        ...
        <div style={{fontFamily:"'Syne',sans-serif",fontSize:28,fontWeight:800,letterSpacing:-1,textAlign:'center',marginBottom:6}}>Create account</div>
        <div style={{fontSize:14,color:'#6b6b85',textAlign:'center',marginBottom:36}}>Start tracking your finances</div>
        ...
        <form onSubmit={handleSubmit}>
          {[{k:'name',l:'Full Name',t:'text',p:'Your name'},{k:'email',l:'Email',t:'email',p:'you@example.com'},{k:'password',l:'Password',t:'password',p:'Min 6 characters'}].map(({k,l,t,p})=>(
          ...
          <button type="submit" disabled={loading} style={{width:'100%',padding:14,borderRadius:12,fontSize:15,fontWeight:700,cursor:'pointer',border:'none',background:'linear-gradient(135deg,#7c5cbf,#9b6dff)',color:'white',fontFamily:"'Syne',sans-serif",marginTop:8,opacity:loading?0.7:1}}>{loading?'Creating...':'Get Started →'}</button>
        </form>
        <div style={{textAlign:'center',marginTop:20,fontSize:14,color:'#6b6b85'}}>Already have an account? <Link to="/login" style={{color:'#9b6dff',fontWeight:600,textDecoration:'none'}}>Sign in</Link></div>
      </div>
    </div>
  );
}
